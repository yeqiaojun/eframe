...
#ifdef WIN32
  //-------------------------------------------------------------  
  #define     EPOLLIN             CPOLLIN                  
  #define     EPOLLOUT            CPOLLOUT                 
  #define     EPOLLRDHUP          CPOLLRDHUP               
  #define     EPOLLPRI            CPOLLPRI                 
  #define     EPOLLERR            CPOLLERR                 
  #define     EPOLLHUP            CPOLLHUP                 
  #define     EPOLLET             CPOLLET                  
  #define     EPOLLONESHOT        CPOLLONESHOT             

  //---------------------------------------------------------------
  #define  EPOLL_CTL_ADD            CPOLL_CTL_ADD             
  #define  EPOLL_CTL_DEL            CPOLL_CTL_DEL             
  #define  EPOLL_CTL_MOD            CPOLL_CTL_MOD             

  typedef cpoll_data        epoll_data;
  typedef cpoll_data_t      epoll_data_t;
  typedef cpoll_event       epoll_event;
  
  #define   epoll_startup()                                     cpoll_startup()   
  #define   epoll_create(size)                                  cpoll_create(size)
  #define   epoll_ctl(epfd, opcode, fd, event)                  cpoll_ctl(epfd, opcode, fd, event)
  #define   epoll_wait(epfd, events, maxevents, timeout)        cpoll_wait(epfd, events, maxevents, timeout);
  #define   epoll_close(cpfd)                                   cpoll_close(cpfd)                                                     
  #define   epoll_cleanup()                                     epoll_cleanup()
  #elif
  
  #include <sys/epoll.h>
  #define   epoll_startup()  
  #define   epoll_cleanup()  
#endif
